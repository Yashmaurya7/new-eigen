...
export function ImageCarouselFold() {
  ...
  return (
    <div className="w-screen bg-black text-white z-10 py-48">
      {/* Heading */}
      <div className="text-center mb-8">
        <h1 className="text-2xl sm:text-6xl font-bold text-blue-600 pb-4">
          Some of Our Research
        </h1>
        <p className="text-white mt-2 text-xs sm:text-2xl pb-12 ">
          Explore our breakthrough innovations in precision medicine and drug discovery
        </p>
      </div>

      {/* Carousel */}
      <div className="relative flex items-center justify-center w-full h-[30vh] sm:h-[70vh]">
        {/* Images */}
        <div className="relative w-full h-full overflow-hidden">
          {images.map((image, index) => (
            <img
        ...
        <div className="absolute bottom-0 w-full px-8 flex justify-center space-x-2">
          {images.map((_, index) => (
            <button
              key={index}
              className={`w-4 h-4 rounded-full ${
